			...
			<div className="container">
				<div className="flex flex-col md:flex-row gap-8">
							...
							<div>
								...
								<div className="space-y-2">
									{data?.meta?.metrics?.categories?.map((cat, index) => (
										<div className="flex items-center justify-between group" key={index}>
											<div className="flex items-center gap-2">
												...
												<span className="group-hover:text-blue-600 transition-colors">
													{cat.category === DocType.lecture_note ? 'Lecture Notes' : 'Past Questions'}
												</span>
											</div>
											<span className="text-xs bg-gray-100  px-2 py-0.5 rounded-full">{cat.total}</span>
										</div>
									))}
								</div>
							</div>
							...
							<div>
								...
								<div className="space-y-2">
									{data?.meta?.metrics?.document_by_levels?.map((dbl, idex) => (
										<div className="flex items-center justify-between group" key={idex}>
											<p className="group-hover:text-blue-600 transition-colors text-sm">{dbl.level} Level</p>
											<span className="text-xs bg-gray-100  px-2 py-0.5 rounded-full">{dbl.total}</span>
										</div>
									))}
								</div>
							</div>
					...
					<div className="flex-1">
							...
							<div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
								...
								<div className="flex items-center gap-4">
									...
									<Select
										...
										<SelectContent>
											<SelectItem value={Sort.most_recent}>Most Recent</SelectItem>
											<SelectItem value={Sort.oldest_first}>Oldest First</SelectItem>
						...
						<div
							className={
								viewMode === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6' : 'space-y-4'
							}>
							{isLoading
								? new Array(10).fill(null).map((_, index) => <DocumentShimmer viewMode={viewMode} key={index} />)
								: data?.data?.map((doc) => <DocumentCard document={doc} key={doc._id} />)}
							...
							{data?.data?.length! > 0 && (
								<Pagination
									...
									current={query?.page}
									prevIcon={query?.page == 1 ? undefined : <ChevronLeftCircle />}
									nextIcon={query?.page == data?.meta?.totalPages ? undefined : <ChevronRightCircle />}
									align="center"
